...
│                                                                                  │
│  ╭──────────╮                                           ╭─────────────────────╮ │
│  │ DevSecOps│  IaC + CI/CD + Security Scanning          │  🤖 FUTURE: AI/ML   │ │
│  ╰──────────╯                                           │  • Fraud Detection  │ │
│                                                          │  • Risk Scoring     │ │
│  ┌─────────────┐   ┌─────────────┐   ┌──────────────┐  │  • GenAI Docs       │ │
│  │   COLLECT   │──▶│  CLEANSE &  │──▶│   CONSUME    │──┤  • NL Queries       │ │
│  │             │   │   CURATE    │   │              │  ╰─────────────────────╯ │
│  │ ┌─────────┐│   │ ┌──────────┐│   │ ┌──────────┐│                           │
│  │ │ Policy  ││   │ │ Mapping  ││   │ │  Query   ││                           │
│  │ │ Claims  ││   │ │Transform ││   │ │  Engine  ││                           │
│  │ │ Billing ││   │ │  Quality ││   │ │  (SQL)   ││                           │
│  │ │ External││   │ │  Lineage ││   │ ├──────────┤│                           │
│  │ └─────────┘│   │ └──────────┘│   │ │Dashboard ││                           │
│  │  Raw files │   │Parquet+Schema│   │ │  (BI)   ││                           │
│  └──────┬──────┘   └──────┬──────┘   └──────┬─────┘│                           │
│         │                  │                  │                                  │
│    ①Upload          ④Transform          ⑧Insight                                │
│         │                  │                  │                                  │
│    ┌────▼────┐       ┌─────▼────┐       ┌────▼────┐                            │
│    │Trigger  │──②──▶│Orchestr. │──⑤──▶│Catalog  │                            │
│    │(Event)  │       │(State M.)│       │(Metadata)│                            │
│    └─────────┘       └──────────┘       └─────────┘                            │
│                           │                                                      │
│                      ┌────▼────┐                                                │
│                      │Key-Value│ Lookups, Audit, PII Vault                      │
│                      │ Store   │                                                │
│                      └─────────┘                                                │
└─────────────────────────────────────────────────────────────────────────────────┘
```

**Componentes genéricos (cloud-agnostic):**
- Object Storage (3 camadas: raw, curated, analytics-ready)
- Serverless ETL Engine (distributed PySpark)
- State Machine (orchestration with retry/error handling)
- Event-driven Trigger (file arrival → pipeline start)
- Key-Value Store (lookups, audit trail, PII tokenization)
- Data Catalog (schema metadata, table discovery)
- Query Engine (SQL on columnar data)
- BI Dashboard (visualizations, calculated KPIs)
- ML/AI Layer (fraud detection, risk scoring, GenAI — future)
